...
#создаем dashboard
#su - ubuntu | git clone https://github.com/kubernetes/heapster.git && kubectl create -f ./heapster/deploy/kube-config/influxdb/ && kubectl create -f ./heapster/deploy/kube-config/rbac/heapster-rbac.yaml && kubectl apply -f https://raw.githubusercontent.com/kubernetes/dashboard/master/src/deploy/alternative/kubernetes-dashboard.yaml
#вываливаем dashboard наружу
